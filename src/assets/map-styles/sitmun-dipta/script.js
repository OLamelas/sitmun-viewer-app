...
/*
 * SITMUN DIPTAV2 - Logo dinámico por territorio
 * ===============================================
 * Objetivo: mostrar en la cabecera del visor el logo específico de cada
 * territorio (municipio) usando su código INE (TER_CODTER).
 *
 * Mecanismo:
 *   1. Extrae appId y territoryId de la ruta: /{public|user}/map/{appId}/{territoryId}
 *   2. Consulta el endpoint /api/config/client/application/{appId}/territories
 *      que solo devuelve los territorios vinculados a esa aplicación (pocos).
 *   3. Localiza el territorio cuyo id numérico coincide con el de la ruta.
 *   4. Cambia el src del elemento .logo-img a /assets/logos/{TER_CODTER}.png.
 *
 * Requisitos:
 *   - Los logos deben estar en front/viewer/sitmun-viewer-app/src/assets/logos/
 *     con el nombre {TER_CODTER}.png (ej: 08019.png, 25120.png, etc.)
 *   - El endpoint /api/config/client/application/{appId}/territories debe ser
 *     accesible (lo es tanto en sección pública como privada porque la misma
 *     API la usa el diálogo de selección de territorio).
 */
(function() {
  'use strict';

  console.log('[sitmun-diptav2] Iniciando detección de logo...');

  /*
   * Extrae appId [1] y territoryId [2] de la URL:
   *   /{public|user}/map/{appId}/{territoryId}
   * Soporta hash routing (#/user/map/3/66) y path routing (/user/map/3/66).
   */
  var match = window.location.hash.match(/\/map\/(\d+)\/(\d+)/)
    || window.location.pathname.match(/\/map\/(\d+)\/(\d+)/);
  console.log('[sitmun-diptav2] hash:', window.location.hash, '| pathname:', window.location.pathname, '| match:', match ? match[0] : null);
  if (!match) {
    console.log('[sitmun-diptav2] No es página de mapa, se cancela');
    return;
  }

  var appId = parseInt(match[1], 10);
  var territoryId = parseInt(match[2], 10);
  console.log('[sitmun-diptav2] appId:', appId, '| territoryId:', territoryId);

  /*
   * Detecta dinámicamente la URL base del API mirando las peticiones
   * que Angular ya ha hecho (ej: el perfil del mapa se cargó antes).
   * Así funciona con cualquier prefijo: /backend/, /stm3/, sin prefijo, etc.
   */
  var apiBase = window.location.origin;
  if (window.performance && window.performance.getEntriesByType) {
    var entries = window.performance.getEntriesByType('resource');
    console.log('[sitmun-diptav2] performance entries encontrados:', entries ? entries.length : 0);
    if (entries) {
      for (var i = 0; i < entries.length; i++) {
        var idx = entries[i].name.indexOf('/api/config/client/');
        if (idx !== -1) {
          apiBase = entries[i].name.substring(0, idx);
          console.log('[sitmun-diptav2] API base detectada por performance:', apiBase);
          break;
        }
      }
    }
  }
  console.log('[sitmun-diptav2] apiBase final:', apiBase);

  /* Lista de URLs a probar en orden */
  var urlsToTry = [
    apiBase + '/api/config/client/application/' + appId + '/territories?size=1000'
  ];
  /* Si la detección dinámica no encontró un prefijo, añade /backend/ como fallback */
  if (apiBase === window.location.origin) {
    urlsToTry.push(window.location.origin + '/backend/api/config/client/application/' + appId + '/territories?size=1000');
    console.log('[sitmun-diptav2] Añadido fallback /backend/');
  }
  console.log('[sitmun-diptav2] URLs a probar:', urlsToTry);

  /*
   * Consulta SOLO los territorios de la aplicación actual.
   * Esto es eficiente porque cada app tiene un número reducido de
   * territorios asociados (tipicamente < 50), a diferencia de
   * /api/config/client/territory que devuelve TODOS los del sistema.
   * Reintenta con la siguiente URL si falla.
   */
  function tryFetch(index) {
    if (index >= urlsToTry.length) {
      console.warn('[sitmun-diptav2] No se ha podido cargar el logo del territorio: todas las URLs fallaron');
      return;
    }
    console.log('[sitmun-diptav2] Intentando fetch URL ' + (index + 1) + '/'
      + urlsToTry.length + ':', urlsToTry[index]);
    fetch(urlsToTry[index]).then(function(response) {
      console.log('[sitmun-diptav2] Respuesta status:', response.status, '| ok:', response.ok);
      if (!response.ok) {
        throw new Error('Status ' + response.status);
      }
      return response.json();
    }).then(function(data) {
      console.log('[sitmun-diptav2] API OK, territorios recibidos:', data.content ? data.content.length : 0);
      var territory = data.content.find(function(t) {
        return t.id === territoryId;
      });
      console.log('[sitmun-diptav2] Territorio buscado id=' + territoryId + ' encontrado:', territory ? 'sí' : 'no');
      if (territory && territory.code) {
        console.log('[sitmun-diptav2] Código INE:', territory.code);
        /*
         * Obtiene la URL del logo desde la configuración del territorio
         * (territorialAuthorityLogo). Si no está definida, construye
         * la ruta a partir del código INE como fallback.
         */
        var basePath = (document.querySelector('base')?.getAttribute('href') || '/').replace(/\/$/, '');
        var logoSrc = null;
        if (territory.territorialAuthorityLogo) {
          var logoUrl = territory.territorialAuthorityLogo;
          if (logoUrl.match(/^https?:\/\//i)) {
            logoSrc = logoUrl;
          } else if (logoUrl.charAt(0) === '/') {
            logoSrc = window.location.origin + logoUrl;
          } else {
            logoSrc = basePath + '/' + logoUrl;
          }
          console.log('[sitmun-diptav2] Logo desde territorialAuthorityLogo:', logoUrl, '->', logoSrc);
        }
        if (!logoSrc) {
          logoSrc = basePath + '/assets/logos/' + territory.code + '.png';
          console.log('[sitmun-diptav2] Sin logo configurado, fallback a código INE:', logoSrc);
        }
        /*
         * Inyecta el logo en la barra de navegación.
         * Si el <img> original (.logo-img) ya existe, cambia su src.
         * Si no (p.ej. porque la app oculta el logo por defecto con
         * logoSitmun.visible = false), lo crea y lo inserta en .toolbar-left.
         */
        var attempts = 0;
        function setLogoSrc() {
          ...
          if (logo) {
            logo.src = logoSrc;
            console.log('[sitmun-diptav2] .logo-img existente actualizado');
            return;
          }
	    ...
	    img.onerror = function() {
            ...
            console.log('[sitmun-diptav2] .logo-img creado e insertado en .toolbar-left');
            return;
          }
          if (attempts < 10) {
            attempts++;
            setTimeout(setLogoSrc, 500);
          } else {
            console.warn('[sitmun-diptav2] No se encontró .toolbar-left tras 10 intentos');
          }
        }
        setLogoSrc();
      } else if (territory && !territory.code) {
        console.warn('[sitmun-diptav2] Territorio encontrado pero sin code (INE):', JSON.stringify(territory));
      }
    }).catch(function(err) {
      console.log('[sitmun-diptav2] Fetch falló, reintentando... Error:', err.message);
      tryFetch(index + 1);
    });
  }
  tryFetch(0);
